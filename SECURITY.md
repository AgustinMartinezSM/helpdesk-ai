...
- `INTERNAL_SERVICE_TOKEN` is the first **service credential** in the platform. auth-service presents it in an `x-internal-service-token` header when it asks organizations-service for a membership while minting a token — the one moment when the pattern used everywhere else (forward the caller's own token, ADR 0011) has nothing to forward. It is deliberately not `JWT_ACCESS_SECRET`: one symmetric key standing for both "this person is authenticated" and "this process is authenticated" would tie the two rotations to each other. organizations-service **requires** it (minimum 32 characters, compared in constant time); on auth-service it is **optional and has no default**, because a default would ship a guessable credential — left unset, membership is not resolved and tokens are minted without organization claims. What the credential opens is one read-only lookup, `GET /internal/memberships/:userId/active`, on a service deliberately absent from the api-gateway routing table: a browser has no path to it, and its answer becomes token claims that no service reads yet.
...
- **Access tokens**: JWT signed with `JWT_ACCESS_SECRET`, 15-minute default TTL, issuer claim, roles embedded. Since Sprint 9.2 the token also carries `org` (the active organization id), `perms` (resolved permission keys) and `mv` (membership version), alongside `sub`, `email`, `roles`, `iss`, `iat` and `exp`; `roles` stays as a compatibility claim. Two things must be read literally: **`perms` is an empty array today** — role templates are still plain strings and the evaluator arrives in a later phase — and **no service reads `org`, `perms` or `mv`**. Every downstream service receives them and ignores them. The three claims are omitted, not null, when no membership resolves, and resolution fails open: if organizations-service is unavailable the token is minted without them and a warning is logged.
...
- **Service credential lifecycle**: `INTERNAL_SERVICE_TOKEN` is compared in constant time and carried in a header, but **rotation is not built and internal calls produce no audit record**. The request log line names the path and the time, so a lookup is traceable for as long as the logs are kept; nothing records which process presented the credential, and nothing reaches the audit trail. ADR 0011 named rotation and an audit record as the story a service credential deserves before one is introduced, and neither is here. Today the credential guards a single read-only lookup whose answer no service acts on, which is why this is an open item rather than a live exposure — it has to be closed before those claims decide anything.
