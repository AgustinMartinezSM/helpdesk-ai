...
and 2 ran in Sprint 9.2, and phases 3 and 4 in Sprint 9.3. Sprint 9.4 is in
progress: phase 6's write half ran first, then phase 5 began — see phase 5 for
why that order is inverted. Each entry records its own deviations. Phases 7
and 8 are unchanged and unstarted.
...
**Still to do:** the users-service directory, the audit filter, the five
analytics aggregates, and the deletion of `isStaff`/`isAdmin` together with
the duplicate `Actor` copies in tickets-service and users-service — which must
land in one change or the copies drift.
...
**Still to do in this phase:** consumers reading v2 (so the rows they project
carry a tenant), notification-service comparing tenant as well as id, the
membership lifecycle, and assignee validation.
