...
| `ticket_snapshots` / `user_snapshots`                             | analytics-service     | Ticket snapshots: per organization via the scoped `GET /tickets`, same shape as ticket_refs. User snapshots: rebuilt by `backfill-user-snapshots.sh` since Sprint 10.7, which reads `helpdesk_organizations.memberships` — the script this cell used to say was missing. It no longer records registrations at all (ADR 0026), so the auth-listing gap stopped applying to this table rather than being closed                                                                                                                                      |
...
| `branch_refs` / `station_refs` / `team_refs` / `team_branch_refs` | tickets-service       | Projections of the structure events, and since Sprint 9.16 the only ones with a rebuild path **inside the platform**: tickets-service reads three read-only, keyset-paginated snapshot endpoints on organizations-service (`/internal/structure/*`, behind `INTERNAL_SERVICE_TOKEN`) and applies them through the same last-write-wins guard the events use. It runs on every boot after the subscription is live, and on demand. No database is crossed — the owner answers over HTTP. Procedure: `docs/architecture/projection-reconciliation.md` |
