...
| auth-service (NestJS)          | 3003                     | Needs `apps/auth-service/.env` (JWT secret, DATABASE_URL, `ORGANIZATIONS_SERVICE_URL` and `INTERNAL_SERVICE_TOKEN` — the last is REQUIRED since Sprint 10.8; without it the process exits at startup naming the variable) and the postgres container. Swagger UI at http://localhost:3003/docs outside production.                                                                                                                                                                                                                                                                           |
| tickets-service (NestJS)       | 3004                     | Needs `apps/tickets-service/.env` (same JWT secret, DATABASE_URL, RABBITMQ_URL; plus `ORGANIZATIONS_SERVICE_URL` + `INTERNAL_SERVICE_TOKEN` or every assignment answers 503 — assignee verification fails closed) plus the postgres and RabbitMQ containers; it publishes the ticket lifecycle events. Swagger at /docs outside production.                                                                                                                                                                                                                                                  |
| users-service (NestJS)         | 3005                     | Needs `apps/users-service/.env` (same JWT secret, DATABASE_URL, RABBITMQ_URL). Consumes `user.registered.v1` and the `membership.*.v1` lifecycle events into its own database — the latter feed the directory_memberships projection that scopes GET /users. Swagger at /docs outside production.                                                                                                                                                                                                                                                                                            |
...
| organizations-service (NestJS) | 3010                     | Needs `apps/organizations-service/.env` (DATABASE_URL, RABBITMQ_URL, INTERNAL_SERVICE_TOKEN) plus the postgres and RabbitMQ containers. **It also requires `JWT_ACCESS_SECRET`** (the same value auth-service signs with) and **is routed by the api-gateway at `/api/organizations`** — both changed in Sprint 9.8 (ADR 0019) when it gained a person-facing surface, and this row said the opposite until Sprint 10.8. A `.env` from before 9.8 fails its boot naming the variable, which is the intent; the symptom upstream is login answering 503. Swagger at /docs outside production. |
...
- organizations-service has to be running for auth-service to stamp the tenant claims (`org`, `perms`, `mv`) on an access token, with `ORGANIZATIONS_SERVICE_URL` pointing at it and the same `INTERNAL_SERVICE_TOKEN` set on both sides. **It is required, and that sentence used to say the opposite.** Since Sprint 10.8 auth-service refuses to boot without `INTERNAL_SERVICE_TOKEN`, and since the tenancy phases it answers **503** on login when organizations-service is unreachable — "I could not ask" is uncertainty, and this service fails closed on uncertainty. Start it whenever you start auth-service, not only when working on tenancy: everything downstream reads those claims.
