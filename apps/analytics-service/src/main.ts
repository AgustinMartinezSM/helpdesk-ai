...
import { SERVICE_NAME, analyticsServiceEnvSchema } from './config/env';
...
async function bootstrap(): Promise<void> {
  ...
  const env = validateEnv(analyticsServiceEnvSchema, process.env);
  ...
  if (env.NODE_ENV !== 'production') {
    const swaggerConfig = new DocumentBuilder()
      ...
      .setDescription('Dashboard projections from domain events')
      .setVersion('0.1.0')
      .addBearerAuth()
      .build();
    SwaggerModule.setup(
