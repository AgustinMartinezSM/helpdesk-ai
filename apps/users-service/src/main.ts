...
import { SERVICE_NAME, usersServiceEnvSchema } from './config/env';
...
async function bootstrap(): Promise<void> {
  ...
  const env = validateEnv(usersServiceEnvSchema, process.env);
  ...
  if (env.NODE_ENV !== 'production') {
    const swaggerConfig = new DocumentBuilder()
      ...
      .setDescription('User profile projection API')
      .setVersion('0.1.0')
      .addBearerAuth()
      .build();
    SwaggerModule.setup(
