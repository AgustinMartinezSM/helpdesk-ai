...
import { SERVICE_NAME, notificationServiceEnvSchema } from './config/env';
...
async function bootstrap(): Promise<void> {
  ...
  const env = validateEnv(notificationServiceEnvSchema, process.env);
  ...
  if (env.NODE_ENV !== 'production') {
    const swaggerConfig = new DocumentBuilder()
      ...
      .setDescription('In-app notifications projected from domain events')
      .setVersion('0.1.0')
      .addBearerAuth()
      .build();
    SwaggerModule.setup(
