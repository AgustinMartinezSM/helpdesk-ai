...
import { SERVICE_NAME, aiServiceEnvSchema } from './config/env';
...
async function bootstrap(): Promise<void> {
  ...
  const env = validateEnv(aiServiceEnvSchema, process.env);
  ...
  if (env.NODE_ENV !== 'production') {
    const swaggerConfig = new DocumentBuilder()
      .setTitle('ai-service')
      .setDescription(
        'Staff-only AI assistance: summaries, classification, priority suggestions and reply drafts',
      )
      .setVersion('0.1.0')
      .addBearerAuth()
      .build();
  ...
  app
    .get(Logger)
    .log(
      `${SERVICE_NAME} listening on http://localhost:${env.PORT} with the ${env.AI_PROVIDER} provider`,
    );
}
