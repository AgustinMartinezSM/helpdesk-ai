...
import { SERVICE_NAME, ticketsServiceEnvSchema } from './config/env';
...
async function bootstrap(): Promise<void> {
  ...
  const env = validateEnv(ticketsServiceEnvSchema, process.env);
  ...
  if (env.NODE_ENV !== 'production') {
    const swaggerConfig = new DocumentBuilder()
      ...
      .setDescription('Ticket lifecycle API')
      .setVersion('0.1.0')
      .addBearerAuth()
      .build();
    SwaggerModule.setup(
